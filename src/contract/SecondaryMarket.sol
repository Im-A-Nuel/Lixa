 ...
 * @notice Simple secondary market for FractionalTokens (ASK-only order book)
 */
contract SecondaryMarket is Ownable, ReentrancyGuard {
    struct Order {
        ...
        uint256 pricePerToken;  // price per token (18 decimals), cost = price * amount / 1e18
        bool active;
        uint256 createdAt;
    }
    ...
    function createSellOrderWithPermit(
        uint256 poolId,
        address ftAddress,
        uint256 amount,
        uint256 pricePerToken,
        ...
        IERC20Permit(ftAddress).permit(msg.sender, address(this), amount, deadline, v, r, s);
        IERC20 ft = IERC20(ftAddress);
        require(ft.balanceOf(msg.sender) >= amount, "insufficient balance");
